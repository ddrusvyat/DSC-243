...
Algorithms compared:
  - SGD with uniform sampling and stepsize gamma = 1/max_i ||d_i||^2
  ...
  - Randomized Kaczmarz (Strohmer-Vershynin): importance-sampling
    p_i = ||d_i||^2 / ||D||_F^2 plus row-adaptive stepsize 1/||d_i||^2 (projection)

Both achieve linear convergence in the interpolation regime; the Kaczmarz rate
sigma_min^2(D) / ||D||_F^2 dominates SGD's rate sigma_min^2(D) / (n * max_i ||d_i||^2)
by the row-norm spread factor max_i ||d_i||^2 / mean_i ||d_i||^2.
"""
...
def main() -> None:
    ...
    ax.semilogy(
        ...
        label=r"SGD (uniform sampling, $\gamma=1/R^2$)",
    )
    ...
    ax.semilogy(
        t_grid, e0 * np.exp(-rate_sgd * t_grid), "--",
        color=color_sgd, linewidth=1.1, alpha=0.7,
        label=r"slope $\mu/R^2$",
    )
    ax.semilogy(
        t_grid, e0 * np.exp(-rate_kac * t_grid), "--",
        color=color_kac, linewidth=1.1, alpha=0.7,
        label=r"slope $\sigma_{\min}^2(D)/\|D\|_F^2$",
    )
    ...
    ax.set_title(
        rf"SGD vs Randomized Kaczmarz on interpolation LS"
