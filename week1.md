...
$$
where $\hat{h}_i := \langle h, \phi_i \rangle_{L^2(\nu)}$ and $\hat{g}_i := \langle g, \phi_i \rangle_{L^2(\nu)}$ are the coefficients of $h$ and $g$ in the eigenbasis. Equivalently,
$$
\sum_i \mu_i^{-2s}\,|\hat{h}_i|^2 \leq M^2.
$$
