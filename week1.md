...
$$p_{k}=r_k+\beta_{k-1} p_{k-1},$$

for a constant $\beta_{k-1}$ to be chosen. We would like to ensure that $p_{k}$ is $A$-orthogonal to $\{p_i\}_{i=0}^{k-1}$. To this end, setting $p_{k}^\top Ap_{k-1}=0$ yields the unique choice of $\beta_{k-1}=-\dfrac{r_k^\top Ap_{k-1}}{p_{k-1}^\top Ap_{k-1}}$. Now for any $i<k-1$ we compute

$$p_{k}^{\top}A p_{i}=r_k^\top A p_{i}+\beta_{k-1}p_{k-1}^{\top}Ap_i.$$

Observe that $p_{k-1}^{\top}Ap_i=0$ by assumed $A$-orthogonality of $\{p_i\}_{i=0}^{k-1}$ and $r_k^\top A p_{i}=0$ because $A p_{i}$ lies in $\mathcal{K}_{i+1}$ and the optimality conditions for $x_k$ imply  $r_k\perp \mathcal{K}_{k}$. Thus $\{p_i\}_{i=0}^{k}$ is indeed an $A$-orthogonal basis for $\mathcal{K}_{k}$.
It remains to declare

$$x_{k+1}=\argmin_{\eta} f(x_k+\eta p_{k}). \tag{6}$$

Indeed, taking the derivative in $\eta$ implies $r_{k+1}\perp p_{k}$ and for any $i<k$ we have orthogonality
...
The convergence guarantees of the previous sections all rely on the assumption $\alpha > 0$---that is, $A$ is positive definite. When $\alpha$ is very close to zero, the condition number $\kappa = \beta/\alpha$ can become arbitrarily large and the linear convergence bounds of Theorems 1, 2, and 3 essentially become vacuous. This situation arises often in practice. As a concrete example let us look at the problem of solving a linear system generated by a kernel matrix.
...
>
> The *Matérn kernel* with smoothness parameter $\nu>0$ and length-scale $\sigma>0$ is
>
> $$k_\nu(x,y)=\frac{2^{1-\nu}}{\Gamma(\nu)}\left(\frac{\sqrt{2\nu}\,\|x-y\|}{\sigma}\right)^\nu K_\nu\!\left(\frac{\sqrt{2\nu}\,\|x-y\|}{\sigma}\right),$$
>
> where $K_\nu$ is the modified Bessel function of the second kind. Three important special cases are:
>
...
>
> In general, the Matérn kernel with parameter $\nu$ is $\lceil \nu\rceil -1$ times continuously differentiable. In the limit $\nu\to\infty$ the Matérn kernel converges to the Gaussian (RBF) kernel $k(x,y)=\exp\bigl(-\tfrac{\|x-y\|^2}{2\sigma^2}\bigr)$, which is $C^\infty$.
>
> By Bochner's theorem, every stationary kernel is the Fourier transform of a nonnegative *spectral measure*. For the Matérn family, the spectral density in $\mathbb{R}^d$ is
>
> $$S(\omega)\propto\left(\frac{2\nu}{\sigma^2}+\|\omega\|^2\right)^{-(\nu+d/2)},$$
>
> so the high-frequency tail decays as $\|\omega\|^{-(2\nu+d)}$&mdash;a polynomial whose exponent increases with smoothness $\nu$. In the Gaussian limit ($\nu\to\infty$) the spectral density becomes $S(\omega)\propto\exp(-\sigma^2\|\omega\|^2/2)$, which decays faster than any polynomial. The rate at which the spectral density decays controls the eigenvalue decay of the kernel matrix. Let $\lambda_1\geq\lambda_2\geq\cdots\geq\lambda_n\geq 0$ denote the eigenvalues of $K$. For data supported on a compact set of intrinsic dimension $p$, the following asymptotic decay rates hold for the eigenvalues $\mu_i$ of the associated integral operator (the kernel matrix eigenvalues satisfy $\lambda_i\approx n\mu_i$):
>
> - **Gaussian kernel:** super-exponential decay, $\mu_i\lesssim C\exp(-c\, i^{2/p})$ for constants $C,c>0$. The kernel is infinitely smooth, so the eigenvalues vanish faster than any polynomial.
> - **Matérn kernel with smoothness $\nu$:** polynomial decay, $\mu_i\asymp i^{-(2\nu+p)/p}$. The RKHS is the Sobolev space $H^{\nu+p/2}$, and higher smoothness gives faster decay.
> - **Laplace kernel** ($\nu=\tfrac{1}{2}$): the slowest Matérn case, $\mu_i\asymp i^{-(1+p)/p}$.
>
> As a consequence, kernel matrices are typically extremely ill-conditioned: the effective condition number $\lambda_1/\lambda_n$ grows rapidly with $n$, and for large $n$ many eigenvalues are numerically zero. This is precisely the regime where the positive definite theory becomes vacuous and the PSD analysis below becomes essential.
>
> The figure below illustrates this phenomenon on synthetic data. We sample $n=5000$ points in $\mathbb{R}^{100}$ and plot the normalized eigenvalues $\lambda_i/\lambda_1$ on a log-log scale. Around index $i\approx d=100$ a sharp transition occurs: the first $d$ eigenvalues correspond roughly to the linear degrees of freedom in the data, while the remaining $n-d$ eigenvalues capture higher-order nonlinear structure and decay at rates governed by the kernel smoothness. Past this threshold the three kernels separate clearly. The Gaussian kernel (blue) exhibits the fastest spectral decay, dropping over $5$ orders of magnitude. The Laplace kernel (red) decays most slowly&mdash;consistent with its minimal smoothness. The Matérn 5/2 kernel (green) is intermediate, matching the polynomial rate $i^{-(2\cdot 5/2+p)/p}=i^{-(5+p)/p}$.
